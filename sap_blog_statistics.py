...
#             self.df_file = pd.read_csv(self.file_name)
#
#     def mt_iter_file(self):
#         if not self.df_file.empty:
#             for index, row in self.df_file.iterrows():
#                 url = row['url']
...
#                 i_class_name = i_element['class'][1]  # Get the second class name
...
#         self.df_results.fillna('0', inplace=True)  # replace NaN
#
...
#         return dict({'df_ok_sum': df_ok_sum, 'df_ok_count': df_ok_count})
